...
export default class PlayerInst extends globalThis.ISpriteInstance {
  ...
  setHasPlayerEnteredLevelEndBox = (val) =>
    (this.#hasPlayerEnteredLevelEndBox = val);
  ...
  pinCameraOffet = (runtime) => {
    ...
    const cameOffset =
      config.levelConfig[runtime.layout.name].cameraFocusOffset;
  ...
  isInWater = (runtime) => {
    const player = this;
    if (
      player.behaviors.Platform.isByWall("left") ||
      player.behaviors.Platform.isByWall("right")
    ) {
      return false;
    }
  ...
  attack = (runtime) => {
    ...
    waitForMillisecond(100).then(() => {
      if (this) {
        this.setAnimationToRun();
        if (isMirrored(this)) {
          runtime.objects.Slash.createInstance(
            config.layers.game,
            this.x - config.SLASH_POSITION_OFFSET,
            this.y,
          );
        } else {
          runtime.objects.Slash.createInstance(
  ...
  secondaryAttack = (runtime) => {
    ...
    if (this.width < 0) {
      const star = runtime.objects.DeathStar.createInstance(
        config.layers.game,
        player.x - 12,
        player.y + 7,
      );
      star.behaviors.Bullet.speed =
        config.DEATH_STAR_SPEED + this.behaviors.Platform.speed;
  ...
  checkEnemyCollisions = (runtime) => {
    ...
    for (const box of runtime.objects.SineFallingBox.instances()) {
  ...
  checkRampCollision = (runtime) => {
    ...
    for (const e of runtime.objects.RampEndZone.instances()) {
      if (e.testOverlap(this)) {
        ...
        waitForMillisecond(50).then(() => {
          if (
            this.animationName != "Shuv" &&
            this.animationName != "Kickflip"
          ) {
